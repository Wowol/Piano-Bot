...
def generate_batch_song(list_all_midi, batch_music=3, start_index=0, fs=30, seq_len=50, use_tqdm=False):
    """
    ...
    print(len(collected_list_input), len(collected_list_target))
...
def generate_dict_time_notes(list_all_midi, batch_song=3, start_index=0, fs=30, use_tqdm=True):
    ...
    dict_time_notes = {}
    process_tqdm_midi = tqdm_notebook(range(start_index, min(start_index + batch_song, len(
        list_all_midi)))) if use_tqdm else range(start_index,  min(start_index + batch_song, len(list_all_midi)))
...
def generate_input_and_target(dict_keys_time, seq_len=50):
    ...
    start_time, end_time = list(dict_keys_time.keys())[
        0], list(dict_keys_time.keys())[-1]
    ...
    for index_enum, time in enumerate(range(start_time, end_time)):
        ...
        if time+1 in dict_keys_time:
            list_append_target.append(','.join(str(x)
                                               for x in dict_keys_time[time+1]))
        else:
...
def _list_of_files_in_directory(directory):
    ...
    return result
generate_batch_song(_list_of_files_in_directory("midi_files"))